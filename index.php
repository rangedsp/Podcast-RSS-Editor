...
<body>
	...
	if (isset($_SESSION['feed'])) {
		$xmlDoc = simplexml_load_string($_SESSION['feed'], 'my_node');
	}
	...
	if (
		...
		$xmlDoc = simplexml_load_file($_FILES['file']['tmp_name'], 'my_node');
		$_SESSION['feed'] = $xmlDoc->asXML();
	}
	...
	if ($_GET['notf']) {
		...
		switch ($_GET['notf']) {
			case '1':
				$notificationContent = $lang[54];
				break;
			case '2':
				$notificationContent = $lang[55];
				break;
			case '3':
				$notificationContent = $lang[56];
				break;
			case '4':
				$notificationContent = $lang[62];
				break;
			case '5':
				$notificationContent = $lang[59];
				break;
			default:
				$notificationContent = "";
		}
	...
	if (!is_null($xmlDoc)) {
		...
		$sinceLastUpdate = ceil((strtotime(time()) - strtotime($thisDate[0])) / 86400);
	...
	if ($ep >= 0) {
		...
		$previewValue = "";
		if (!is_null($xmlDoc)) {
			$previewValue =	htmlspecialchars($xmlDoc->asXML());
		}

		$settingsContent = "
		<form method='post' autocomplete='on' action='' enctype='multipart/form-data' >
			<section>
				<label for='fileContent'>$lang[73]</label>
				<textarea name='fileContent' style='width:80%;height:auto;' >$previewValue</textarea>
			</section>
			<section>
				<label for='file'>$lang[39]</label>
				<input name='file' type='file' />
			</section>
			<br />
			<input type='submit' name='submit' value='$lang[41]' class='right-in-10'>
		</form>
	";
	} else {
		$panelTitle = "$lang[42]";
	...
	<section class="notification <?php echo ($showNotification); ?>">
		<div class="inner"><?php echo ($notificationContent); ?></div>
	</section>
	...
	<nav>
		...
		<ul>
			...
			<?php
			if (!is_null($xmlDoc)) {
				$encodedContent = base64_encode($_SESSION['feed']);
				$downloadLink = "<li><a download='feed.xml' href='data:application/octet-stream;base64,$encodedContent'>$lang[74]</a></li>";
				echo ($downloadLink);
			}
			?>
		</ul>
	...
	<article class="panel edit-panel">
		<aside>
			<a href='?ep=-1' class="item add-new-item">
		...
		<div class="editor">
			...
			<form action="index.php?ep=<?php echo $ep ?>" method="post">
				...
				<section class="edit-title right-in-2">
					<h3><?php echo $lang[7] ?> </h3><span><input type="text" name="newTitle" value="<?php echo $currentTitle ?>" /></span>
				</section>
				<section class="edit-date right-in-3">
					<h3><?php echo $lang[8] ?> </h3>
					<span><input type="text" name="newMonth" value="<?php echo $currentMonth ?>" /><label for="newMonth"><?php echo $lang[45] ?></label><input type="text" name="newDay" value="<?php echo $currentDay ?>" /><label for="newDay"><?php echo $lang[46] ?></label><input type="text" name="newYear" value="<?php echo $currentYear ?>" /><label for="newYear"><?php echo $lang[47] ?></label><input type="text" name="newHour" value="<?php echo $currentHour ?>" /><label for="newHour"><?php echo $lang[48] ?></label><input type="text" name="newMinute" value="<?php echo $currentMinute ?>" /><label for="newMinute"><?php echo $lang[49] ?></label><input type="text" name="newSecond" value="<?php echo $currentSecond ?>" /><label for="newSecond"><?php echo $lang[50] ?></label>
					</span>
				</section>
				<section class="edit-duration right-in-4">
					<h3><?php echo $lang[9] ?> </h3>
					<span><input type="text" name="newHH" value="<?php echo $currentHH ?>" /><label for="newHH"><?php echo $lang[51] ?></label><input type="text" name="newMM" value="<?php echo $currentMM ?>" /><label for="newMM"><?php echo $lang[52] ?></label><input type="text" name="newSS" value="<?php echo $currentSS ?>" /><label for="newSS"><?php echo $lang[53] ?></label>
					</span>
				</section>
				<section class="edit-link right-in-5">
					<h3><?php echo $lang[10] ?><?php if ($currentLink) {
													echo ("<em><a target=\"_blank\" href=\"$currentLink\">$lang[72]</a> ⎋</em>");
												} ?>
					</h3>
					<span><input type="text" name="newLink" value="<?php echo $currentLink ?>" /></span>
				</section>
				<section class="edit-author right-in-6">
					<h3><?php echo $lang[11] ?> </h3><span><input type="text" name="newAuthor" value="<?php echo $currentAuthor ?>" /></span>
				</section>
				<section class="edit-image right-in-7">
					<h3><?php echo $lang[12] ?><?php if ($currentImage) {
													echo ("<em><a target=\"_blank\" href=\"$currentImage\">$lang[72]</a> ⎋</em>");
												} ?>
					</h3>
					<span><input type="text" name="newImage" value="<?php echo $currentImage ?>" /></span>
				</section>
				<section class="edit-audio right-in-8">
					<h3><?php echo $lang[13] ?> <?php if ($currentFile) {
													echo ("<em><a target=\"_blank\" href=\"$currentFile\">$lang[72]</a> ⎋</em>");
												} ?></h3>
					<span><input type="text" name="newFile" value="<?php echo $currentFile ?>" /></span>
				</section>
				<section class="edit-desc right-in-9">
					<h3><?php echo $lang[14] ?> </h3><span><textarea name="newDesc" /><?php echo $currentDesc ?></textarea></span>
				</section>
			</form>
		</div>
	</article>
